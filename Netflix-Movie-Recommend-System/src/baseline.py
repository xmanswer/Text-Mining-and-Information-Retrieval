...
Edited for homework 6: baseline performances of memory based mthod, model based
...
__sweepD__ = True #disable/enable D sweep in PMF
...
for k in K:
    ...
    for f in os.listdir(cdir):
        if f.startswith('exp'):
            stdout = subprocess.check_output("python eval_ndcg.py dev.csv " + f + " dev.golden")
            print f + ':\t' + stdout
...
for d in D:
    ...
    print "PMF_score for D = ' + str(d) + ':\t" + subprocess.check_output("python eval_ndcg.py dev.csv " + f + " dev.golden")
